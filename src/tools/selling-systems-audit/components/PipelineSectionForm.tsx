...
import type {
  PipelineAnswers,
  PipelineVolumeAnswers,
  PipelineVelocityAnswers,
  PipelineForecastingAnswers,
...
import { ProgressBar, StepNav, StepHeader } from "./StepShell";
import {
  Chips,
  Segmented,
  MaturitySpectrum,
...
export type PipelineSectionFormProps = AuditSectionFormProps<PipelineAnswers> & {
  /** Echo of the typical sales cycle, sourced by the host from Conversion. */
  cycleEcho: string | null;
};

/**
 * Presentational Pipeline Health form. No auth, no supabase, no router —
 * everything comes in through props (see `sectionFormProps.ts`).
 */
export function PipelineSectionForm({
  title,
  draftAnswers,
  ...
  // Hydrate once, after the host can persist and the row has loaded.
  useEffect(() => {
    if (hydrated || !canPersist || isLoading) return;
    const d = (draftAnswers ?? {}) as PipelineAnswers;
