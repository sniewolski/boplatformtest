...
export function useActivityIntake(auditId: string | undefined) {
  return useQuery({
    queryKey: ["ssa-activity-intake", auditId],
    enabled: !!auditId,
    staleTime: 5 * 60_000,
    gcTime: 30 * 60_000,
...
export function useSaveDraft(ownerId: string | undefined, auditId: string | undefined) {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: ["ssa-activity-intake", auditId] });
    },
  });
}
