...
export function useProcessIntake(auditId: string | undefined) {
  return useQuery({
    queryKey: ["ssa-process-intake", auditId],
    enabled: !!auditId,
    staleTime: 5 * 60_000,
    gcTime: 30 * 60_000,
...
export function useSaveDraft(ownerId: string | undefined, auditId: string | undefined) {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: ["ssa-process-intake", auditId] });
    },
  });
}
